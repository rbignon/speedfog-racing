...
A zone is backtracked when the player turns away from it to take another path: its `zone_history` entry immediately precedes a `type: "backtrack"` entry (the warp landing recorded on death/teleport/quit-out). Two exclusions keep the metric honest about intent:

- **Warp chains:** consecutive `"backtrack"` entries are one warp chain; intermediate landings are never credited, only the zone the player originally left counts.
- **Runbacks:** if the chain ends with the player back in the same zone (any landing in the chain, or the first entry after it), the player did not change path (typical death-at-boss runback), so nothing is counted. A chain with no later entry does count: no return happened before the race ended.
