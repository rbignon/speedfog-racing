...
- `[display]`: metadata shown in the web UI (estimated_duration, description, type, sort_order, rules). `rules` is an optional multi-line string; each non-empty line becomes a "Mode Rules" bullet, shown in the rules card above the DAG on `/race/[id]` and `/daily/[date]`, in the download modal, and in the pool settings card's "Mode Rules" popover.
