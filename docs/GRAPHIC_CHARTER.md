...
**Brass usage (exhaustive list):** the "Racing" half of the wordmark, section header device + title, primary CTA button, the hero underline's and the DAG's start/terminal markers, the week line's playing-state segment and dot, the route lines' riding segment, position dot and boarded ring (the viewer's own run), the current-user marker in participant lists (left border + tint), the `Participating` role mark's square, 1st-place markers (`1st` labels in data rows, the ring on winner avatars), the amber "running" daily strip, the chat's transient new-messages jump pill, the dates inside the event page's format prose (the paragraph's only emphasis besides its bold opening). Nowhere else. (The week line's own edge markers are state-colored, not brass.)

**Fog usage:** interactive hover states (outline buttons, nav icons, quiet links), links, focus rings, boss diamonds on the DAG, the `Organizing` role mark's diamond, radio/checkbox checked states (outside player-colored contexts). Route-line cards are the exception: they hover in their own line's status hue, not fog. Fog never marks "you" in the web UI: structural own-run marks (participant lists, the week leaderboard row, the caster list's badge) ride brass, keeping fog a transient interaction accent (the fog fill on your own reaction pill is a checked state, not a "you" mark). The one static exception is the event page's champion plate, once a tournament is over: fog border, ring and soft glow (`--glow-fog`) so the winner stands out from the brass the rest of the page is built on; the newcomers' winner beside it stays neutral. The in-game overlay is the documented exception (fog is the local player's accent there).
...
The panel is a readout, not a window: it carries no widget and takes no mouse input at all, so a click over it never reaches ImGui, which used to answer one anywhere on its body for want of a title bar. `overlay.anchor` and the `overlay.position_offset_x` / `overlay.position_offset_y` margins are how it is placed.

#### Countdown

The race countdown is the one moment the overlay leaves its corner and takes the screen, because it lands on the character screen the players wait on rather than on live gameplay: a brass ring emptying clockwise from 12 o'clock (the arc's end stays pinned at 12 while its start walks round, the way a clock hand erases behind itself), the digit crossfading on every tick (the entering one settles from ×1.25 while the one it replaces swells to ×1.5 and fades, the site's countdown dezoom at frame rate), a brass shockwave on the last three ticks, and a page-ground vignette closing in on the screen edges. Zero flips the slot to a verdigris `GO!` on a verdigris blast, and everything the countdown had put on screen lifts within 0.4s so the race opens on a clean image. The ring and the digit sit centered in the upper half of the screen (a quarter of the way down, horizontally centered), while the vignette closes in on all four edges alike. Nothing labels the composition: the player is one keypress from a race they chose and the panel already names it, so the digit is the only thing worth that much screen. The status line keeps its own countdown column throughout: it is the whole countdown when `overlay.fullscreen_countdown` is off.
