...
**Fog usage:** interactive hover states (outline buttons, nav icons, quiet links), links, focus rings, boss diamonds on the DAG, the `Organizing` role mark's diamond, radio/checkbox checked states (outside player-colored contexts). Route-line cards are the exception: they hover in their own line's status hue, not fog. Fog never marks "you" in the web UI: structural own-run marks (participant lists, the week leaderboard row, the caster list's badge) ride brass, keeping fog a transient interaction accent (the fog fill on your own reaction pill is a checked state, not a "you" mark). The in-game overlay is the documented exception (fog is the local player's accent there).
