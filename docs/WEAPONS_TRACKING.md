...
`server/data/weapons.json` is the static name/type table, generated by `tools/generate_weapons.py` from the game's own data (`EquipParamWeapon.wepType` in `regulation.bin`, `WeaponName` FMGs in `msg/engus/item_dlc02.msgbnd.dcx`). The `wep_type` values are `EquipParamWeapon.wepType`; TarnishedTool's `WepType` enum, the original source of this table (`Properties/Resources.resx`), uses the same numbering, and the generated table reproduced that snapshot exactly. Keyed by base `EquipParamWeapon` row ID:
...
The runtime ID at any affinity and upgrade is `base_id + affinity * 100 + upgrade_level`. 487 rows as of Elden Ring 1.17 (Tarnished Pack: Leontiel's Greatsword, Hefty Scimitar, Golden Order Flail, Silver Grooved Shield, Ritual Thrusting Shield, Reverse-Bladed Sword, Reed Great Katana, Idus Sword).
...
When a patch adds new weapons, regenerate the catalogue from the patched game files with speedfog's `game_inspect` tool (runs under Wine on Linux, see speedfog's `CLAUDE.md`):

1. Dump `EquipParamWeapon` and the item FMGs from the patched, unpacked game directory:

   ```bash
   cd $SPEEDFOG_PATH/tools/game_inspect
   wine publish/win-x64/game_inspect.exe dump-param <game>/regulation.bin EquipParamWeapon \
       --all --field wepType --defs ../../writer/FogModWrapper/eldendata/Defs > params.txt
   wine publish/win-x64/game_inspect.exe dump-fmg <game>/msg/engus/item_dlc02.msgbnd.dcx > fmg.txt
   ```

2. Regenerate the JSON; the script prints the added, removed and changed rows against the previous file:

   ```bash
   python tools/generate_weapons.py --params params.txt --fmg fmg.txt
   ```

   A row is kept when it is a base row (ID ending in four zeros), has a `WeaponName` entry, and is neither Unarmed (`wepType` 33) nor ammunition. Unnamed base rows (`[ERROR]` in the FMG, `sortId` 9999999) are NPC-only variants and stay out.
...
4. Run `tests/test_weapons.py` and `tools/test_generate_weapons.py`. The `test_filter_rejects_shield_and_torch` test enumerates every excluded `wep_type` against the JSON and fails if a category becomes unrepresented in the snapshot.
