...
The current `graph.json` (v2) describes zone-level connections for the mod. The DAG visualization requires node-level data. The speedfog generator will be updated to produce an enriched format.
...
1. **X-axis:** determined by layer number. Spacing between layers is proportional to the weight of the destination node (a legacy_dungeon with weight 23 creates a wider gap than a boss_arena with weight 2). Weight is on the node, so the gap is _before_ the station (representing time spent traversing it).
...
| **Countdown** | Fake, blurred       | Fake, blurred       | Fake, blurred | Fake, blurred             | Real                          | Real             |
...
- Generated procedurally from pool config parameters (typical layer count, node count, branching factor) — not from the actual seed, to avoid leaking information
- The server does NOT send `graph_json` to unauthorized clients
- The fake DAG only needs to have a plausible silhouette when blurred
...
### 3.2 New Field on Race: `organizer_participates`

```python
class Race(Base):
    # ...existing fields...
    organizer_participates: Mapped[bool] = mapped_column(default=False)
```

- Set at race creation, **immutable** after that
- If `True`: organizer is auto-added as a participant, sees blurred DAG
- If `False`: organizer sees real DAG, cannot later join as participant
- Enforced server-side: reject `POST /api/races/{id}/participants` if it would add the organizer and `organizer_participates` is `False`
...
- Adds a caster to the race (creates user + invite if not in DB, same pattern as participants)
- Organizer only
- Can be done at any race status

```
DELETE /api/races/{id}/casters/{user_id}
```

- Removes a caster from the race
- Organizer only
...
New field: `organizer_participates`. When `true`, the server auto-creates a Participant record for the organizer.
...
- **Upcoming Races**: races with status `open` or `countdown`. Cards show participant slots (e.g., "6/8 players").
...
- **Active race spotlight**: if the user has a race in `running` or `countdown` status (as participant, organizer, or caster), it's promoted to a large card with direct CTAs.
- **My Races grid**: horizontal cards sorted by recency. Shows role (Organizing / Participating), status, and result for finished races (e.g., "2nd / 4").
...
- Meta-stats visible in clear below DAG: "12 layers · 18 nodes · 6 paths"
...
- **Race stats**: total duration, average deaths, most deadly zone, etc.
...
The spectator WebSocket at `/ws/race/{race_id}` currently requires no authentication. To support role-based DAG visibility, it gains **optional** cookie-based authentication:

- The existing Twitch OAuth flow already sets an `api_token` cookie
- On WebSocket connect, the server checks for a valid cookie
- If authenticated: resolve the user's role (organizer, participant, caster) to determine DAG access
- If not authenticated: treat as anonymous spectator

This is backward-compatible: unauthenticated connections still work, they just get anonymous spectator access.
...
`total_nodes` and `total_paths` are extracted from `graph_json` at seed scan time and stored as new columns on the `Seed` model:

```python
class Seed(Base):
    # ...existing...
    total_nodes: Mapped[int] = mapped_column(Integer, nullable=False)
    total_paths: Mapped[int] = mapped_column(Integer, nullable=False)
```

This avoids parsing `graph_json` at runtime when it must not be sent.

When `graph_json` is `null`, the frontend renders the fake blurred DAG using the meta-stats to generate a plausible silhouette.
...
- Add `Caster` table, `organizer_participates` field
- Add `GET /api/users/search` endpoint
- Add caster CRUD endpoints
- Update race creation to handle `organizer_participates`
...
- Race stats (duration, deaths, most deadly zone)
- Share functionality
...
### Step 8: graph.json v3 Integration

- Update speedfog generator to produce enriched format (separate project)
- Update server seed scanning to handle v3 format
- Update seed pack generation
- Replace hand-picked hero seed with real v3 data
...
- Deterministic: assigned in order of joining (first participant = color 0, etc.)
...
Until v3 is available from the generator, a hand-crafted JSON file based on the spoiler.txt node data can be used.

### 12.3 graph.json v2 Compatibility

Steps 2–7 can work with **v2 graph.json** data by deriving node information:

- Nodes can be inferred from `area_tiers` (each unique area becomes a node)
- Layers come from `area_tiers` values (mapped to sequential layer numbers)
- Edges come from `connections`
- Node types, weights, and display_names are **unavailable** in v2 — use defaults (all nodes as `mini_dungeon`, uniform weight, raw area IDs as names)

This allows the metro DAG renderer to be developed and tested before the speedfog generator produces v3. The v3 integration (Step 8) then unlocks the full visual richness.
...
Existing overlay routes from Phase 1 design (`/overlay/{id}/dag`, `/overlay/{id}/leaderboard`) are not implemented yet. Phase 2 scope:
...
Both use the same components as the main race page but with `background: transparent` and no chrome. Casters and organizers see direct links to these on the race detail page.

### 12.6 Countdown Duration

The `countdown_seconds` field in `race.config` defaults to **10 seconds** if not specified. Not exposed in the creation form for now (hardcoded default). Can be made configurable later if organizers want longer countdown.
