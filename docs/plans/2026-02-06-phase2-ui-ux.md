...
The server determines whether the organizer is participating by checking for a Participant record matching `race.organizer_id`. This derived check is used for:

- DAG visibility (Section 2.5): organizer sees real DAG only if they are NOT a participant
- Preventing the organizer from adding themselves later: reject `POST /api/races/{id}/participants` if the target user is the organizer and a Participant record already exists (handled by the existing unique constraint), or if the organizer is not a participant and tries to add themselves (would grant them DAG access they shouldn't have after seeing it — blocked server-side)

**Rule:** once an organizer has created a race without participating (and therefore has seen the real DAG), they cannot later add themselves as a participant. Enforced by: reject adding the organizer as participant if they are not already one.
...
The spectator WebSocket at `/ws/race/{race_id}` currently requires no authentication. To support role-based DAG visibility, it gains **optional** token-based authentication via query parameter:

```
/ws/race/{race_id}?token={api_token}
```

- The `api_token` is stored in the browser's `localStorage` (set during Twitch OAuth)
- The frontend includes it as a query parameter when connecting the WebSocket
- On WebSocket connect, the server checks for a valid `token` query param
- If authenticated: resolve the user's role (organizer, participant, caster) to determine DAG access
- If absent or invalid: treat as anonymous spectator

This is backward-compatible: connections without a `token` param still work, they just get anonymous spectator access.
...
- Add `Caster` table, `total_nodes`/`total_paths` on Seed, `color_index` on Participant
...
- Update server seed scanning to handle graph.json v3 format
- Update WebSocket `race_state` to conditionally include/omit `graph_json`
- Add optional token-based auth on spectator WebSocket
