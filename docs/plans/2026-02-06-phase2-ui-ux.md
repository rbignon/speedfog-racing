...
- Rework `POST /api/races/{id}/start`: remove `scheduled_start` body, implement countdown via `asyncio.create_task` (Section 12.5)
...
### 12.5 Countdown Mechanism

The countdown is a fixed **10 seconds** (hardcoded, not configurable for now).

**Server-side flow:**

1. Organizer calls `POST /api/races/{id}/start` (no body — the `scheduled_start` field is removed)
2. Server sets race status to `COUNTDOWN` and broadcasts `race_status_change: countdown`
3. Server launches `asyncio.create_task` that sleeps 10 seconds, then:
   - Sets race status to `RUNNING`
   - Broadcasts `race_start` to mods and `race_status_change: running` to spectators

This is a simple fire-and-forget async task (no job queue, no persistence). If the server restarts during the 10-second window, the race stays in `COUNTDOWN` and the organizer must re-trigger the start.

**Client-side:** the mod and frontend both receive `race_status_change: countdown`, display a visual countdown, and then react to `race_start` / `race_status_change: running` when it arrives.
