...
1. **X-axis:** determined by layer number. Spacing between layers is proportional to the weight of the source node (a legacy*dungeon with weight 23 creates a wider gap \_after* it than a boss_arena with weight 2). The gap is after the station, representing the time spent traversing that dungeon before reaching the next one. Example: chapel_start (w:1) → short gap → academy (w:12) → long gap → next node.
...
Complete role × state matrix. "Real" = actual graph_json from seed. "Fake" = procedurally generated, blurred.
...
1. **Drawing phase** (0–2s): metro lines trace progressively left-to-right, layer by layer. Stations appear as lines reach them. Technique: SVG `stroke-dasharray` + `stroke-dashoffset` CSS animation.
2. **Racing phase** (looping): 3–4 colored dots traverse the DAG at different speeds, taking different paths. One finishes first, another gets overtaken. Technique: SVG `<animateMotion>` along paths, or JS keyframe animation for more control (pauses when "exploring" a zone).
...
```
┌─────────────────────────────────────────────────────────────────┐
│  SpeedFog Racing                            [Login with Twitch] │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│                 ┌─ HERO: ANIMATED DAG ─────────────────┐        │
│                 │                                      │        │
...
│                 └──────────────────────────────────────┘        │
│                                                                 │
│          Competitive Elden Ring racing through the fog          │
│    Race on randomized worlds. Track progress in real-time.      │
│                                                                 │
│                     [Login with Twitch]                          │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
...
│  🔴 Live Races                                                  │
│  ┌──────────────────────┐  ┌──────────────────────┐            │
│  │ Sunday Showdown  LIVE│  │ Casual Friday    LIVE│            │
│  │ 4 players · Standard │  │ 3 players · Sprint   │            │
│  │ by Roger             │  │ by Someone           │            │
│  └──────────────────────┘  └──────────────────────┘            │
│                                                                 │
│  Upcoming Races                                                 │
...
- **Upcoming Races**: races with status `open`. Cards show participant count (e.g., "6 players").
...
```
┌─────────────────────────────────────────────────────────────────┐
│  SpeedFog Racing          [avatar] Roger  [Create Race] [logout]│
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─ ACTIVE RACE (if any) ─────────────────────────────────┐    │
│  │  🔴 Sunday Showdown                         LIVE        │    │
│  │  Standard · 4 players · Layer 6/12                       │    │
│  │  [Spectate]  [Manage]                                    │    │
│  └──────────────────────────────────────────────────────────┘    │
│                                                                 │
│  My Races                                                       │
│  ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐   │
│  │ Test Race  DRAFT│ │ Last Week  FIN  │ │ Old Race   FIN  │   │
│  │ Organizing      │ │ Participated    │ │ Organizing       │   │
│  │ 0 participants  │ │ 2nd / 4         │ │ 6 participants   │   │
│  └─────────────────┘ └─────────────────┘ └─────────────────┘   │
│                                                                 │
│  ── or if no races: ──                                          │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  No active race.  [Create Race] or browse below          │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  🔴 Live Races                                                  │
│  (same as non-connected, filtered to exclude user's own)        │
│                                                                 │
│  Upcoming Races                                                 │
│  (same as non-connected)                                        │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────┐
│  Header: race name, status badge, elapsed time (if running)  │
├──────────────┬───────────────────────────────────────────────┤
│   SIDEBAR    │              MAIN AREA                        │
│              │                                               │
│  (evolves   │     DAG (blurred, live, or complete)           │
│   per state) │                                               │
...
┌ ─ ─ ─ ─ ┐
  + Invite
└ ─ ─ ─ ─ ┘
┌──────────┐  ← appears when + Invite clicked
│🔍 search │     (organizer only)
│ results..│
└──────────┘
...
- **Header**: race name, 🔴 LIVE badge, elapsed time (clock)
...
- **Casters** section: channels with "🔴 live" indicator if they are streaming
...
│  (●) Yes, I'll race    ( ) No, organize only                 │
│                                                               │
│  ⚠ If you choose "organize only", you will see the DAG       │
│    and cannot join as a player later.                         │
│                                                               │
