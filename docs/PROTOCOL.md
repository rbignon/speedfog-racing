...
| `quit_out`        | `boolean?`                  | Optional, omitted when false; marks a post-quit-out reload so the server resumes the participant's current zone (logged as an info line, "quit-out detected, resuming node_id=...") instead of running the resolution heuristics. Falls back to those heuristics (trusting the reported position) if the stored current zone is no longer part of the seed's graph (e.g. after a reroll) |
...
| `countdown`           | Waiting  | Race is in countdown phase (event flags not yet accepted); for the status_update-driven codes, the server re-sends the error on every rejected message while the condition holds, but `countdown` is emitted once per rejected `event_flag`, so its line shows as a short flash rather than persisting | "Race countdown in progress"                                   |
| `session_inactive`    | Waiting  | Training session ended (finished, abandoned, or cancelled) while the mod is still connected                                                                                                                                                                                                            | "Solo session not active"                                      |

**Display logic:** The mod caches the most recent error receipt timestamp and renders the coded condition with a persistent red banner (blocking codes) or calm amber line (waiting codes) if the timestamp is younger than 3 seconds. This 3-second window allows errors to persist visibly as the server re-sends them every ~1 second, but drop automatically after resolution without requiring a separate clear message. Unknown or absent codes fall back to legacy behavior: a transient 3-second display. Old mod builds ignore the `code` field entirely.
...
3. **Mod (overlay):** The status line's right side reflects the race state: gold "WAITING" (setup), countdown digits then green "GO!" for 3 seconds (start), then the running IGT (green once finished). During setup the overlay also shows an amber "Race has not started yet" waiting line, only while the player is in the world (never on the title screen or during loading screens). On top of that column, the countdown also plays fullscreen (digits, draining ring, vignette, `GO!` burst; see the charter's in-game overlay section), unless `overlay.fullscreen_countdown` is set to `false`. Both are local: they run off the `race_start` message's `countdown_seconds`, so a mod that authenticates mid-countdown shows neither and simply starts on the running IGT.
