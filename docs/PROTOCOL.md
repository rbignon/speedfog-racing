...
**System messages** (`role: "system"`): Ephemeral notifications generated by the server when a player finishes, abandons, or is auto-removed due to inactivity. These are broadcast to the public channel only and are not persisted to the database (they won't appear in `chat_history` on reconnect).
...
The server broadcasts system notifications to the public channel for the following events:

| Event                          | Message                                                |
| ------------------------------ | ------------------------------------------------------ |
| Player finishes                | `"{display_name} has finished the race!"`              |
| Player abandons                | `"{display_name} has abandoned the race."`             |
| Player auto-removed (inactive) | `"{display_name} has been removed due to inactivity."` |

System messages use `role: "system"` with empty `username`, `null` `display_name` and `avatar_url`. They are ephemeral (not persisted to the database), so they do not appear in `chat_history` on reconnect.

### Persistence

User-sent messages are persisted to the database (indexed by `race_id`, `channel`, `created_at`) and replayed as `chat_history` on connection. System messages are broadcast-only.
