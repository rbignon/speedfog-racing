...
When the server sends a `zone_update`, the mod stores it in `pending_zone_update` and stamps `pending_zone_received_at`. While a zone update is pending (the loading byte and the CSMenuManImp screen-state/blackscreen chains are only read from game memory on those frames):
