...
**`finish_event` is one-shot**: Uses `triggered_flags` (HashSet), so the player can only finish once. Preserved across reconnects (so a finish that was sent before disconnect doesn't fire again on reconnect rescan). Cleared on a detected **save reload** so a stale save's pre-set finish_event doesn't block the fresh save's real finish: each frame, if the live IGT regresses by more than `SAVE_RELOAD_IGT_DROP_MS` from the previous reading, the mod assumes the player loaded a save and flushes `triggered_flags`, `deferred_event_flags`, and `pending_event_flags`. Elden Ring's IGT is monotonic while a save is in memory; only a menu load repopulates GameDataMan from disk, so a regression always means a save load. Note this includes ordinary quit-outs of the same save: the on-disk value lags the last in-memory reading by the post-flush fade time (live-measured above 1s), which is also one of the two quit-out detection signals (see the quit-out arming in `race_machine.rs`).
