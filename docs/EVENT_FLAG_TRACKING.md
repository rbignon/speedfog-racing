...
**`finish_event` is one-shot**: Uses `triggered_flags` (HashSet), so the player can only finish once. Preserved across reconnects (so a finish that was sent before disconnect doesn't fire again on reconnect rescan). Cleared on a detected **save reload** so a stale save's pre-set finish_event doesn't block the fresh save's real finish: each frame, if the live IGT regresses by more than 60 seconds (`SAVE_RELOAD_IGT_DROP_MS = 60_000`) from the previous reading, the mod assumes the player loaded a save and flushes `triggered_flags`, `deferred_event_flags`, and `pending_event_flags`. Zero readings are ignored for all reload classification: GameDataMan's play_time transiently reads 0 while a menu reload repopulates it, so 0 carries no information (a real racing save's IGT is never 0 when readable). Elden Ring's IGT is monotonic while a save is in memory; only a menu load repopulates GameDataMan from disk, so a regression always means a save load. The 60-second threshold is chosen to distinguish between legitimate backup restores (regressions larger than 60s, accepted by the server) and quit-out reloads: on a quit-out of the same save, the on-disk IGT value lags the last in-memory reading by the post-flush fade time (typically 1-2 seconds). The quit-out penalty arms only for regressions strictly under 60 seconds; at 60 seconds or more the per-save flag flush fires instead, preventing the mod from re-sending flags for progress the restored save no longer has: the server accepts large rollbacks by design (backup restores are legitimate), so a stale report would be recorded, not bounced.
...
When the server sends a `zone_update`, the mod stores it in `pending_zone_update` and stamps `pending_zone_received_at`. While a zone update is pending, the loading byte is read from game memory (otherwise it is skipped); the CSMenuManImp screen-state/blackscreen chains are not exclusive to a pending reveal, they are also read continuously mid-run for the IGT freeze. Reveal behavior while pending:

- Primary signal: reveal once `position_readable && screen_in_game && !blackscreen` (the CSMenuManImp screen state, direct in-game read with no fade covering it): move `pending_zone_update` to `current_zone` (displayed on overlay). This is immune to the world-clock failure modes below and also reveals correctly on frozen-clock event seeds, since it never depends on the loading byte.
- Fallback: when the direct signal is unavailable (`screen_in_game` is `None`: unmapped build or unreadable pointer chain), reveal once the loading byte is clear (or unreadable) AND the position is readable, as before.
- Defensive timeout `ZONE_REVEAL_TIMEOUT = 5s` from receipt: on the fallback path, frozen-clock event seeds (weather plugin FreezeTime, seeds generated before the 2026-07-15 generator fix) never clear the byte, so the reveal falls back to the timeout, still gated on a readable position (a reveal never fires before the world is loaded). Logged as `warn!("Zone revealed (timeout, reveal signal stuck)")`, the log marker for those seeds. FreezeTime seeds now reveal normally at load exit whenever the direct signal is available, since it does not depend on the world clock.
...
| `ZONE_REVEAL_TIMEOUT`    | 5s         | `core/race_machine.rs`  | Reveal fallback when the reveal signal is stuck |
