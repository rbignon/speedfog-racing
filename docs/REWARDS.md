...
- **Stability is a correctness requirement, not a nicety**: the mod starts one apply runner per skin name, and a draw that moved mid-run costs a runner swap in-game (see [PHANTOM_SKINS.md](PHANTOM_SKINS.md#lifecycle)). The player would watch their aura change colour mid-race for no reason they can see.
...
- The chrome typography rule applies to **all UI chrome** (navbar, headings, body, labels, badges, buttons, leaderboard data columns): the three charter faces (display / UI / data) and nothing else. Only the player's _name itself_, when rendered through a name template, may use an alternative font.
- The "no animation" rule (sole exception: Rage Quit) extends to name templates: no shimmer, no pulse, no animated gradient.
- The color palette outside name templates is unchanged: brass (`#C8A44E`), fog (`#A99BC9`), text colors, semantic status colors, etc.
...
Restricted, mapped to badge "kind". This palette predates charter v2 (it still uses the v1 purple and greys); retune it to the current accent set the next time badge assets are touched.

| Use                    | Hex       | Tone                            |
| ---------------------- | --------- | ------------------------------- |
| Champion / top tier    | `#C8A44E` | Gold (charter accent)           |
| Veteran / longevity    | `#9CA3AF` | Steel grey                      |
| Contributor / craft    | `#A78BFA` | Light purple (legacy v1 accent) |
...
| `contributor`           | `#A78BFA` (head) + `#5B21B6` (handle) | Craft / authorship, ties to the legacy v1 purple. Bicolor: see exception above                |
...
- `font-weight` (limited to `400` / `500` / `600`, the UI-face weights bundled by the app)
- `font-style` (`normal` | `italic`)
- `font-family` **only** among:
  - `"Public Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif` (charter default; same stack as the rest of the UI)
