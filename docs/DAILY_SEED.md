...
| Streak rollback     | n/a                                       | Every user who had qualified for this `daily_date` has their streak state re-derived from scratch via `rollback_streak_for_reroll` (see [specs/2026-05-12-daily-streak-design.md](specs/2026-05-12-daily-streak-design.md)). `best_streak` is preserved as a high water mark. |
...
- `StreakIndicator.svelte`: small inline component rendered next to the `Daily Seed` title on `/`, `/dashboard`, and `/daily/[date]` (above the `DailyWeekGrid`). It shows `🔥 {N}-day streak` when `my_streak.current > 0`, plus a `· ❄️ {F} freeze[s]` segment when `freeze_count > 0`. Hidden below 640px width so it does not crowd the mobile layout (the dedicated `/daily/[today]` page surfaces the same info).
