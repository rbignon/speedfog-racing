...
- `DailyWeekGrid.svelte`: rendered on `/` (variant `home`, replaces the previous banner) and on `/dashboard` (variant `dashboard`, replaces the previous "Today + recent dailies" section). Shows seven cells for the current calendar week (Monday through Sunday in ISO order) with one of four states per cell: `missing_past` (no race row for that past day), `past` (clickable, podium top 3 + finishers count + viewer's result if logged in), `today` (uses the standard neutral border with a small gold "TODAY" header badge, plus a state-driven bottom strip), `future` (greyed out, pool from `daily_seed_schedule` + "Opens in HH:MM" countdown). The today cell always carries a full-width bottom strip whose label and color depend on the viewer's participant status: "PLAY NOW" (green) when anonymous or `my_result == null`, "IN PROGRESS" (orange) for `registered`/`ready`/`playing`, "FINISHED N/Total" (green) when finished, "ABANDONED" (muted grey) when abandoned. The `.me` "You: ..." footer is suppressed on today, since the strip carries that signal. Past cells (with `my_result != null`) keep the green left-edge accent ("played" indicator); the stripe is past-only. Mobile (under 640px) switches the grid to a horizontal-scroll strip with the today cell auto-centered on mount.
