...
- `/daily/[date]/+page.svelte`: the dedicated landing page for a given rotation date. The same component renders the live daily and any past daily; it branches on whether `now < race_ends_at`. Layout mirrors `/race/[id]` with daily-specific adaptations (single public chat tab, no `RaceStatus` badge, no participants list during setup). While the daily is still open, the DAG area carries the `JoinRaceCta` component for a viewer who is not yet a participant: the future map slot, filled with a ghost path silhouette behind the call to action ("Play now" here, "Join race" / "Join now" on `/race/[id]`, which renders the same component while the race is joinable). A logged-out click stores the current path as `redirect_after_login` and leaves for Twitch login. The `DownloadModal` is reused with `actionLabel = "Download Seed Pack"` so the same flow lands the per-participant zip. Between the `DailyWeekGrid` and the DAG, the page renders a read-only copy of the race page's rules card: a "Mode Rules" section listing the pool's `rules` lines, then, on a deathless daily, a "Race Rules" section with the deathless rule (dailies have no custom rules, so that is its only line). The card is omitted when neither section has content. On a deathless daily the header also carries a `Deathless` chip, and the modal opens with its ember `Deathless` callout, as on any deathless race.
...
| ------------------------------------ | ------------------------------------- | -------------- |
| Non-logged or non-participant        | `JoinRaceCta` ("Play now")            | `MetroDagFull` |
