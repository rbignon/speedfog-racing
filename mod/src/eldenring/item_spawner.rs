...
//!    (not a fresh New Game). The spawner waits for MapItemMan to become null
//!    (player quit to title) then retries on the next game load.
...
/// for each item.
///
/// If the save is stale (IGT > 15s), waits for the player to quit back to
/// the title screen (MapItemMan becomes null) and retries on the next game
/// load. This handles the case where the player loads an old save, gets
/// rejected, then starts a New Game without the WS connection dropping
/// (which would otherwise never re-trigger spawning via auth_ok).
///
...
pub fn spawn_items_blocking(
    ...
    loop {
        ...
        // save instead of starting a New Game. Wait for them to quit back to the
        // title screen (MapItemMan becomes null) and retry on the next game load.
        let igt_ms = pointers.igt.read().map(|v| v as u32).unwrap_or(0);
        if igt_ms > MAX_FRESH_IGT_MS {
            warn!(
                igt_ms,
                max = MAX_FRESH_IGT_MS,
                "Stale save detected (IGT too high), waiting for fresh save..."
            );
            // Wait for MapItemMan to become null (player quit to title screen)
            let mut last_null_log = std::time::Instant::now();
            loop {
                let p = unsafe { pp.read() };
                if p.is_null() {
                    break;
                }
                if last_null_log.elapsed() > Duration::from_secs(60) {
                    info!("Still waiting for player to quit to title screen...");
                    last_null_log = std::time::Instant::now();
                }
                std::thread::sleep(Duration::from_millis(500));
            }
            info!("MapItemMan null, retrying spawn on next game load");
            continue;
        }
