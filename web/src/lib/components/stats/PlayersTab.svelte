<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      ...
      error =
        e instanceof Error ? e.message : "Failed to load player profiles.";
    } finally {
      loading = false;
    }
  }
  ...
  let activeSections = $derived(
    TRAITS.filter((t) => getPlayers(t.key).length > 0),
  );
</script>
    ...
    {#each activeSections as trait}
      ...
      <section class="trait-section">
        <div class="trait-header">
          ...
          <span class="trait-name" style="color: {trait.color}"
            >{trait.label}</span
          >
