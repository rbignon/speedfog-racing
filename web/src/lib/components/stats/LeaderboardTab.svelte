<script lang="ts">
  ...
  function templateFor(player: LeaderboardPlayer) {
    const id = player.equipped_name_template_id;
    if (!id || id === "default") return null;
    return rewards.lookupTemplate(id);
  }

  function nameStyleFor(player: LeaderboardPlayer): string {
    const t = templateFor(player);
    const parts: string[] = [];
    if (t?.gradient) {
      parts.push(
  ...
  function rowStyleFor(player: LeaderboardPlayer): string {
    const t = templateFor(player);
      ...
      <table class="ranking-table">
        ...
        <tbody>
          {#each players as player, i}
            {@const badge = rewards.lookupBadge(player.equipped_badge_id)}
            <tr style={rowStyleFor(player)}>
              ...
              <td class="player-cell">
                ...
                {/if}
                <a
                  href="/user/{player.twitch_username}"
                  class="player-name"
                  style={nameStyleFor(player)}
                >
