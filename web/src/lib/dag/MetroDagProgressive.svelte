<script lang="ts">
  ...
  let visibleNodes: PositionedNode[] = $derived(
    filterVisibleNodes(layout.nodes, visibility),
  );
  ...
  let playerPath = $derived.by(() => {
    ...
    return {
      segments: waypointSegments.map((seg) =>
  ...
  >
    ...
    {#each visibleEdges as edge (edge.fromId + "-" + edge.toId)}
      <g
        ...
        opacity={edgeOpacity(edge, visibility, EDGE_OPACITY)}
      >
