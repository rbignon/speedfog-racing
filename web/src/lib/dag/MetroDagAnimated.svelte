<script lang="ts">
  ...
  let nodeTimings: NodeAppearTiming[] = $derived(
    computeNodeAppearTimings(layout, edgeTimings),
  );
  ...
  {#if layout.nodes.length > 0}
    ...
    >
      ...
      {#each layout.edges as edge}
        ...
        {@const totalEdgeLen = edge.segments.reduce(
          (s, seg) => s + segmentLength(seg),
          0,
        )}
        {#each edge.segments as seg, segIdx}
          {@const dashLen = segmentLength(seg)}
          {@const segFractionBefore =
            edge.segments
              .slice(0, segIdx)
              .reduce((s, prev) => s + segmentLength(prev), 0) /
            (totalEdgeLen || 1)}
      ...
      {#if showRacers}
        {#each racerPositions as pos, i}
          <circle
            ...
            class="racer-dot"
            style="filter: drop-shadow(0 0 4px {HERO_RACER_COLORS[
              i % HERO_RACER_COLORS.length
            ]});"
          />
        {/each}
      {/if}
