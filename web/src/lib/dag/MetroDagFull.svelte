<script lang="ts">
  ...
  let adjacency: DirectedAdjacency = $derived(
  ...
  let playerPaths: PlayerPath[] = $derived.by(() => {
    ...
    for (const p of participants) {
      ...
      expandedMap.set(
        p.id,
        expandNodePath(deduped, edgeMap, adjacency, visited, dedupedTypes),
      );
    }
    ...
    for (const p of participants) {
      ...
      const segments = waypointSegments.map((seg) =>
        seg.map((w) => `${w.x},${w.y}`).join(" "),
      );
  ...
  let trailingPaths: TrailingSegment[] = $derived.by(() => {
    ...
    for (const p of participants) {
      ...
      for (const entry of recent) {
        if (
          deduped.length === 0 ||
          deduped[deduped.length - 1].node_id !== entry.node_id
        ) {
...
{#snippet dagContent()}
    ...
    {#each playerPaths as path (path.id)}
      {#each path.segments as seg}
        <polyline
          ...
          opacity={hasHighlight && !highlightIds!.has(path.id)
            ? 0
            : fullPathOpacity
              ? 1
              : 0.8}
  ...
  {#each layout.nodes as node}
    <g
      class="dag-node"
      class:selected={popupData?.nodeId === node.id}
      data-type={anonymous ? undefined : node.type}
    ...
    >
      ...
      <g class="dag-node-shape">
        ...
        {:else if node.type === "start"}
          ...
          />
          <polygon
            points="{node.x - 3},{node.y - 5} {node.x - 3},{node.y +
              5} {node.x + 5},{node.y}"
            fill={BG_COLOR}
          />
