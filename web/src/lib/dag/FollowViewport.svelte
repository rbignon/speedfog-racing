<script lang="ts">
  ...
  let maxX = $derived(
    nodeMap.size > 0
      ? Math.max(...[...nodeMap.values()].map((n) => n.x))
      : width,
  );
  ...
  let targetViewport: Viewport = $derived.by(() => {
    const activePlayers =
      raceStatus === "finished"
        ? participants.filter(
            (p) => p.status === "finished" || p.status === "playing",
          )
    ...
    const avgLayer =
      playerLayers.reduce((s, l) => s + l, 0) / playerLayers.length;
  ...
  let offscreenIndicators = $derived.by(() => {
    ...
    const visibleStatuses =
      raceStatus === "finished" ? ["playing", "finished"] : ["playing"];
  ...
  {#if width > 0 && height > 0}
    ...
    {#each offscreenIndicators as ind (ind.participantId)}
      <div
        ...
        style="--player-color: {ind.color}; top: {Math.max(
          5,
          Math.min(95, ind.yPercent),
        )}%;"
      >
        <span class="offscreen-chevron"
          >{ind.side === "left" ? "\u25C0" : "\u25B6"}</span
        >
