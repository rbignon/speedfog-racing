<script lang="ts">
  ...
  let replayParticipants = $derived(
    buildReplayParticipants(participants, graphJson),
  );
  let maxIgt = $derived(
    Math.max(...replayParticipants.map((rp) => rp.totalIgt), 0),
  );
  ...
  let commentaryEvents = $derived(
    mapHighlightsToTimeline(highlights, replayParticipants),
  );
  ...
  function openPopupForNode(nodeId: string) {
    ...
    if (dagContainer) {
      const el = dagContainer.querySelector(
  ...
  function seek(progress: number) {
    ...
    if (
      replayState === "idle" ||
      (replayState === "finished" && progress < 1)
    ) {
      replayState = "paused";
    }
  }
  ...
  let progress = $derived(
    REPLAY_DEFAULTS.DURATION_MS > 0
      ? replayElapsedMs / REPLAY_DEFAULTS.DURATION_MS
      : 0,
  );
      ...
      <div class="replay-svg-area">
        ...
        >
          ...
          <DagBaseLayer
            {layout}
            {labelAbove}
            selectedNodeId={popupData?.nodeId}
          />
            ...
            <span class="commentary-text">
              {#each activeCommentary.segments as seg}
                ...
                {:else if seg.type === "player"}
                  <span
                    class="commentary-player"
                    style="color: {playerColor(seg.playerId)}">{seg.name}</span
                  >
