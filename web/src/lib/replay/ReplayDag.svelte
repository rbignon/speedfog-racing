<script lang="ts">
  ...
  let leaderChanged = $derived(
    leaderId !== previousLeader && previousLeader !== null,
  );
  ...
  function skullScale(progress: number): number {
    ...
    if (progress < 0.5) {
      ...
      return (
        REPLAY_DEFAULTS.SKULL_PEAK_SCALE - ((progress - 0.3) / 0.2) * overshoot
      );
    }
    return 1.0;
  }
...
{#each snapshots as snap (snap.participantId)}
  ...
  {#if rp}
    <circle
      ...
      class="replay-dot"
      filter={isHidden || isGhost || isFrozen
        ? undefined
        : "url(#replay-player-glow)"}
    >
