<script lang="ts">
  ...
  let graphJson = $derived(
    liveSeed?.graph_json ?? data.session.graph_json ?? null,
  );

  let dagParticipants = $derived.by(() => {
    if (liveParticipant) return [liveParticipant];
    if (!data.session.zone_history || data.session.zone_history.length === 0)
      return [];
    return [
      {
        id: data.session.id,
        ...
        status:
          data.session.status === "active"
            ...
            : data.session.status,
        current_zone:
          data.session.zone_history[data.session.zone_history.length - 1]
            ?.node_id ?? null,
