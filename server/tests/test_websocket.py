...
class TestGapComputation:
    ...
    def test_compute_gap_exceeded_budget(self):
        ...
        # gap = entry_delta + layer_overshoot = 5000 + 5000 = 10000
        gap = compute_gap_ms(
            "playing",
            igt_ms=130000,
            current_layer=2,
    ...
    def test_compute_gap_exceeded_budget_ahead_player(self):
        ...
        # gap = -5000 + 10000 = 5000 (player was ahead but lost more than the lead)
        gap = compute_gap_ms(
            "playing",
            igt_ms=125000,
            current_layer=2,
    ...
    def test_compute_gap_never_exceeds_igt(self):
        ...
        # Player has only spent 5000ms in the layer, leader spent 10000ms
        # entry_delta = 90000, but player is within leader's time budget
        gap = compute_gap_ms(
            "playing",
            igt_ms=105000,
            current_layer=1,
            player_layer_entry_igt=100000,
            leader_splits=leader_splits,
            leader_igt_ms=0,
        )
...
class TestSharedEntranceDedup:
    """Test deduplication of multi-flag shared entrance events."""
...
async def _resolve(equipped: str | None) -> str | None:
    """Call the resolver on the paths that never reach the database."""
    return await resolve_phantom_skin_for_auth_ok(
...
class TestPhantomSkinResolution:
    """Test resolve_phantom_skin_for_auth_ok, the equip value -> wire value map.

    The ``random`` expansion it delegates to is covered in
    test_rewards_service.py, next to the unlock table it reads.
    """

    async def test_resolves_none_to_none(self):
        assert await _resolve(None) is None

    async def test_resolves_string_none_to_none(self):
        """The mod must receive null, never the literal "none"."""
        assert await _resolve("none") is None

    async def test_passes_through_real_skin_id(self):
        assert await _resolve("gold-aura") == "gold-aura"
